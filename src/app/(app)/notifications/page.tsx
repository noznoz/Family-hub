...
import { getNotifications, getUpcomingReminders } from '@/lib/journey-queries';
import { markAllRead } from '@/lib/actions/notifications';
import { Card } from '@/components/ui/card';
import { EmptyState } from '@/components/ui/empty-state';
import { Button } from '@/components/ui/button';
import { UpcomingReminders } from '@/components/notifications/upcoming-reminders';
...
export default async function NotificationsPage() {
  ...
  const [items, reminders] = session.isDemo
    ? [[], []]
    : await Promise.all([
        getNotifications(session.memberId),
        getUpcomingReminders(session.familyId, session.memberId),
      ]);
  ...
  return (
    <div className="space-y-4">
      ...
      <UpcomingReminders reminders={reminders} live={!session.isDemo} />
