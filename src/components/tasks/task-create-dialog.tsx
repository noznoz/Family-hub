...
export function TaskCreateDialog({
  ...
  const onSubmit = (formData: FormData) => {
    ...
    startTransition(async () => {
      const res = await createTask({ title, description, priority, dueDate, studentId, assigneeId });
      if (!res.ok) {
        setError(res.error);
        return;
      }
      ...
      <DialogContent title="New task">
        <form action={onSubmit} className="space-y-3">
          <Field label="Title" htmlFor="title">
            <Input id="title" name="title" required placeholder="e.g. Pay tuition (Term 1)" />
          </Field>
          ...
          {!live && (
            <p className="rounded-lg bg-brand-muted px-3 py-2 text-xs text-navy">
              Demo mode — added locally only. Connect Supabase to persist tasks.
            </p>
          )}
