...
import { createRecipe } from '@/lib/actions/support';

const CATEGORIES = ['breakfast', 'lunch', 'dinner', 'snacks', 'quick', 'family_favorites'];

export function RecipeCreateDialog({ live }: { live: boolean }) {
  ...
  const onSubmit = (formData: FormData) => {
    ...
    startTransition(async () => {
      ...
      setOpen(false);
      if (res.id) router.push(`/support/recipes/${res.id}`);
      else router.refresh();
    });
  };
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild><Button variant="brand" size="sm"><Plus className="size-4" /> New</Button></DialogTrigger>
      <DialogContent title="New recipe">
        <form action={onSubmit} className="space-y-3">
          <Field label="Name" htmlFor="name"><Input id="name" name="name" required placeholder="e.g. Chicken & Rice" /></Field>
          ...
          <div className="grid grid-cols-2 gap-3">
            <Field label="Category" htmlFor="category">
              <Select id="category" name="category" defaultValue="family_favorites" className="capitalize">
          ...
          <div className="grid grid-cols-3 gap-3">
            <Field label="Prep (min)" htmlFor="prep"><Input id="prep" name="prep" type="number" min="0" inputMode="numeric" /></Field>
            <Field label="Cook (min)" htmlFor="cook"><Input id="cook" name="cook" type="number" min="0" inputMode="numeric" /></Field>
            <Field label="Serves" htmlFor="servings"><Input id="servings" name="servings" type="number" min="0" inputMode="numeric" /></Field>
          </div>
          <Field label="Ingredients (one per line)" htmlFor="ingredients">
            <textarea id="ingredients" name="ingredients" rows={4} placeholder={'2 chicken breasts\n1 cup rice'} className="flex w-full rounded-xl border border-input bg-white px-4 py-2 text-base focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" />
          </Field>
          <Field label="Steps (one per line)" htmlFor="steps">
            <textarea id="steps" name="steps" rows={4} placeholder={'Fry the onion\nAdd chicken'} className="flex w-full rounded-xl border border-input bg-white px-4 py-2 text-base focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" />
          </Field>
          ...
          <div className="flex gap-2 pt-1">
            ...
            <Button type="submit" variant="brand" className="flex-1" disabled={pending}>{pending ? 'Saving…' : 'Create recipe'}</Button>
          </div>
        </form>
      </DialogContent>
    </Dialog>
